    ...
    $.getJSON(dao, function (response) {
        ...
        if(tipoPruebaSol === '5'){//MUNSELL
            ...
            for (var j = 0; j < response.data.length; j++) {
                ...
                var tdReporte = document.createElement("td");
                let reporte = response.data[j].reportePieza;
                let enlace = document.createElement('a');
                tdReporte.appendChild(enlace);
...
function llenarEstatusPrueba(estatus) {
    $.getJSON(rutaBase + '/dao/daoEstatusPrueba.php', function (data) {
        ...
        for (var j = 0; j < data.data.length; j++) {
            var currentEstatus = data.data[j].id_estatusPrueba;

            // Filtrar los estatus cuando estatus sea igual a 1
            if (estatus === 1 && ![1, 2, 5].includes(currentEstatus)) {
                continue; // Saltar si el estatus no es 1, 2 o 5
            }

            var createOption = document.createElement("option");
            createOption.value = currentEstatus;
            createOption.text = data.data[j].descripcionEstatus;
            selectS.appendChild(createOption);

            // Si el valor actual coincide con id_estatusSol, se selecciona por defecto
            if (currentEstatus === estatus) {
                createOption.selected = true;
            }
        }
    });
